...
func (c *Compiler) parseMakeInterface(val llvm.Value, typ types.Type, global string) (llvm.Value, error) {
	...
	itfTypeCode := c.createRuntimeCall("makeInterface", []llvm.Value{itfTypeCodeGlobal, itfMethodSetGlobal}, "typecode")
...
func (c *Compiler) getTypeCode(typ types.Type) llvm.Value {
	...
	if global.IsNil() {
		global = llvm.AddGlobal(c.mod, c.ctx.Int16Type(), typ.String()+"$type")
		global.SetGlobalConstant(true)
	}
	return global
}
...
func (c *Compiler) getMethodSignature(method *types.Func) llvm.Value {
	...
	if signatureGlobal.IsNil() {
		signatureGlobal = llvm.AddGlobal(c.mod, c.ctx.Int16Type(), "func "+signature)
