...
func (p *lowerInterfacesPass) run() {
	...
	for _, use := range interfaceMethodUses {
		...
		if len(itf.types) == 0 {
			...
			paramTypes[len(params)-1] = p.ctx.Int16Type()
	...
	for _, use := range interfaceImplementsUses {
		...
		if !actualType.IsACallInst().IsNil() && actualType.CalledValue() == makeInterface {
			...
			assertedType := p.builder.CreateLoad(typ.typecode, "typeassert.typecode")
		...
		if len(itf.types) == 0 {
			...
			// constant.
			p.builder.SetInsertPointBefore(use)
			assertedType := p.builder.CreateLoad(itf.types[0].typecode, "typeassert.typecode")
			commaOk := p.builder.CreateICmp(llvm.IntEQ, assertedType, actualType, "typeassert.ok")
			use.ReplaceAllUsesWith(commaOk)
			use.EraseFromParentAsInstruction()
		} else {
	...
	for i, t := range typeSlice {
		t.num = uint64(i + 1)
		t.typecode.SetInitializer(llvm.ConstInt(p.ctx.Int16Type(), t.num, false))
		t.typecode.SetLinkage(llvm.PrivateLinkage) // should be optimized away
	}
	...
	for _, use := range makeInterfaceUses {
		...
		use.ReplaceAllUsesWith(t.typecode.Initializer())
		use.EraseFromParentAsInstruction()
	}
	...
	for _, use := range typeAssertUses {
		...
		if t.countMakeInterfaces == 0 {
			...
			// regular type assert
			p.builder.SetInsertPointBefore(use)
			commaOk = p.builder.CreateICmp(llvm.IntEQ, assertedTypeGlobal.Initializer(), actualType, "typeassert.ok")
		}
		use.ReplaceAllUsesWith(commaOk)
		use.EraseFromParentAsInstruction()
	}
...
func (p *lowerInterfacesPass) getInterfaceImplementsFunc(itf *interfaceInfo) llvm.Value {
	...
	fnType := llvm.FunctionType(p.ctx.Int1Type(), []llvm.Type{p.ctx.Int16Type()}, false)
...
func (p *lowerInterfacesPass) createInterfaceImplementsFunc(itf *interfaceInfo) {
	...
	for _, typ := range itf.types {
		sw.AddCase(typ.typecode.Initializer(), thenBlock)
	}
...
func (p *lowerInterfacesPass) createInterfaceMethodFunc(itf *interfaceInfo, signature *signatureInfo) {
	...
	for _, typ := range itf.types {
		...
		sw.AddCase(typ.typecode.Initializer(), bb)
